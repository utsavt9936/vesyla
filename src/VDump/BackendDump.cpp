 ...
 *
// This method gets a loop and according to its location in the nested loop,
// creates a list in descending order (from higher degree to lower degree).
void VDump::createLoopList(BIR::LoopHeaderInstruction *currLoop_, LoopList &loopList_)
{
	LoopHeaderInstruction *upperLoop = currLoop_;

	loopList_.clear();

	while (upperLoop)
	{
		loopList_.insert(loopList_.begin(), upperLoop);
		upperLoop = upperLoop->upperLoop;
	}
}
...
void VDump::drraCodeGeneration(BIR::DRRACluster *drraCluster_, VIR::MainProgram *mainProgram_)
{
	...
	SetCurrentFile(ftMakeFirstRun);
	dumpMakeFirstRunFile();
	RestoreCurrentFile();

	SetCurrentFile(ftMakeRerun);
	dumpMakeRerunFile();
	RestoreCurrentFile();
	...
	vesyla::filegen::FileGenerator g;
	g.generate(drraCluster_);
}

// This method dumps different parts of a DRRA cluster
void VDump::dumpDRRACluster(BIR::DRRACluster *drraCluster_)
{
	// SetCurrentFile(ftInstructions);
	// dumpInstructions(drraCluster_);
	// RestoreCurrentFile();

	// SetCurrentFile(ftSchedules);
	// dumpScheduledOperations(drraCluster_);
	// RestoreCurrentFile();
	...
	// SetCurrentFile(ftOptimizedSchedules);
	// dumpOptimizedSchedule(drraCluster_);
	// RestoreCurrentFile();
}
...
/* // REMOVE:
// This method dumps a DiMArch instruction
void VDump::dumpDimarchInstruction(BIR::Instruction * instruction_, int index_)
{
	wtToken("\n");
	...
	{
		case bktRouteInstruction:	dumpRouteInstruction((RouteInstruction *) instruction_); break;

		case bktSRAMInstruction:	dumpSramInstruction((SRAMInstruction *) instruction_);	 break;

		case bktRFILEInstruction:	dumpRfileInstruction((RFILEInstruction *) instruction_); break;
	}
...
void VDump::dumpInstructions(BIR::DRRACluster *drraCluster_)
{
	...
	{
		dumpTitle("DiMArch Sequencer Instructions", "#");
		...
		for (int colIndex = 0; colIndex < dimarch->colNum(); ++colIndex)
		{
			...
			for (int instIndex = 0; instIndex < instList->size(); ++instIndex)
				dumpDimarchInstruction(instList->instruction(instIndex), instIndex);
		}
	} */
}
...
void VDump::dumpMatlabCode(BIR::DRRACluster *drraCluster_, VIR::MainProgram *mainProgram_)
{
	...
	// COMMENTED TO DISABLE USELESS LOGGING
	//WriteLine("fileID1 = fopen('mt_results1_exc_" + InputFileName() + ".txt','w');");
	//WriteLine("fileID2 = fopen('mt_results2_reg_" + InputFileName() + ".txt','w');");

	WriteLine("fileID3 = fopen('mt_results_" + InputFileName() + ".txt','w');");
	WriteLine("");
	// COMMENTED TO DISABLE USELESS LOGGING
	//WriteLine("fprintf(fileID2,'\\n##-------------- Initial Values --------------##');");

	WriteLine("");

	// COMMENTED TO DISABLE USELESS LOGGING
	//_finalMatlabCodes = "% Final Variable Value Printing: ";
	//_finalMatlabCodes += "\nfprintf(fileID2,'\\n\\n##--------------- Final Values ---------------##');";
	...
	// COMMENTED TO DISABLE USELESS LOGGING
	//WriteLine("");
	//WriteLine(_finalMatlabCodes);
	//WriteLine("");

	// COMMENTED TO DISABLE USELESS LOGGING
	//WriteLine("fclose(fileID1);");
	//WriteLine("fclose(fileID2);");
...
{
	vector<VIR::Expression *> &parameters = primitiveFunction_->parametersSplit();

	string printAssignStr = "printAssignStatementPrimFunc" + to_string(parameters.size()) + "(fileID1, ";

	printAssignStr += "'" + primitiveFunction_->name()->name() + "', ";

	for (int pIndex = 0; pIndex < parameters.size(); ++pIndex)
	{
		Expression *expr = parameters[pIndex];
		switch (expr->kind())
		{
		...
		case ktIdentifier:
		{
			...
			sliceName->prefix((Identifier *)expr);
			sliceName->suffix().push_back(CREATE_INTEGER(1));

			printAssignStr += "'";
			printAssignStr += dumpMatlabExpression(sliceName->prefix()) + "', ";
			printAssignStr += dumpMatlabExpression(sliceName->suffix()[0]) + ", '";
			printAssignStr += dumpMatlabExpression(sliceName->suffix()[0]) + "', ";
			printAssignStr += dumpMatlabExpression(sliceName, true) + ", ";
		}
		break;

		case ktSliceName:
		{
			SliceName *sliceName = (SliceName *)expr;
		...
		{
			printAssignStr += "'";
			printAssignStr += dumpMatlabExpression(expr) + "', ";
			printAssignStr += dumpMatlabExpression(expr) + ", '";
			printAssignStr += dumpMatlabExpression(expr) + "', ";
			printAssignStr += dumpMatlabExpression(expr) + ", ";
		}
		...
		default:
			LOG(FATAL) << ("dumpMatlabPrimitiveFunc: Unsupported primitive function!");
			break;
		}
	}
	...
	if (outputNames_.size() > 0)
	{
		...
		if (outputNames_.size() == 1)
		{
			printAssignStr += "{'";
			printAssignStr += dumpMatlabExpression(outputNames_[0]->prefix()) + "'}, [";
			printAssignStr += dumpMatlabExpression(outputNames_[0]->suffix()[0]) + "], {'";
			printAssignStr += dumpMatlabExpression(outputNames_[0]->suffix()[0]) + "'}, [";
			printAssignStr += dumpMatlabExpression(outputNames_[0], true) + "]";
		}
		else
		{
			for (int i = 0; i < outputNames_.size(); i++)
			{
				...
				{
					printAssignStr += "{'";
					printAssignStr += dumpMatlabExpression(outputNames_[i]->prefix()) + "', '";
				}
				else if (i == (outputNames_.size() - 1))
				{
					printAssignStr += dumpMatlabExpression(outputNames_[i]->prefix()) + "'}, ";
				}
				else
			...
			for (int i = 0; i < outputNames_.size(); i++)
			{
				...
				{
					printAssignStr += "[";
					printAssignStr += dumpMatlabExpression(outputNames_[i]->suffix()[0]) + ", ";
				}
				else if (i == (outputNames_.size() - 1))
				{
					printAssignStr += dumpMatlabExpression(outputNames_[i]->suffix()[0]) + "], ";
			...
			for (int i = 0; i < outputNames_.size(); i++)
			{
				...
				{
					printAssignStr += "[";
					printAssignStr += dumpMatlabExpression(outputNames_[i], true) + ", ";
				}
				else if (i == (outputNames_.size() - 1))
				{
					printAssignStr += dumpMatlabExpression(outputNames_[i], true) + "] ";
	...
	// COMMENT TO ALLOW ANY PRIMITIVE FUNCTION
	//	if (parameters.size() != 1 && primitiveFunction_->type() != ftMac && primitiveFunction_->type() != ftSymmetricMac &&
	//		primitiveFunction_->type() != ftSubAcc && primitiveFunction_->type() != ftAbsSubAcc && primitiveFunction_->type() != ftPreservingMac &&
	//		primitiveFunction_->type() != ftShiftLeft && primitiveFunction_->type() != ftShiftRight && primitiveFunction_->type()!=ftButterfly)
	//		LOG(FATAL) << ("dumpMatlabPrimitiveFunc: Illegal arguments!");

	switch (parameters.size())
	{
	// case of MAC/SubAcc/AbsSubAcc functions
	case 2:
		printAssignStr += (primitiveFunction_->type() == ftMac ? ", '.*'" : ", '-'");
		break;

	// case of Symmetric MAC function
	case 3:
		printAssignStr += ", '.*', '+'";
		break;
	...
	printAssignStr += ", " + isFixedPointStr_ + ");";

	return printAssignStr;
}
...
void VDump::dumpMatlabStatement(VIR::Statement *statement_)
{
	...
	case ktForStatement:
	{
		...
		// Instrumenting the code with the print command
		// COMMENTED TO DISABLE USELESS LOGGING
		//WriteLine("printForStatement(fileID1, '" + forStatement->loopVariable()->name() + "', " + forStatement->loopVariable()->name() + ");");
	...
	case ktAssignmentStatement:
	{
		...
		case atDeclarative:
		{
			...
			// COMMENTED TO DISABLE USELESS LOGGING
			//string printStr = "printVariable(fileID2, " + assignStatement->lhsIdentifiers()[0]->name() +
			//									(isFixedPointVariable(assignStatement->lhsIdentifiers()[0]) ? ".double" : "") + ", '" + assignStatement->lhsIdentifiers()[0]->name() + "', " +
			//									(isFixedPointVariable(assignStatement->lhsIdentifiers()[0]) ? "1" : "0") + ");";
			//WriteLine(printStr);
			//WriteLine("");
			//
			//_finalMatlabCodes += "\n" + printStr;
		...
		case atArithmetic:
		{
			string isFixedPointStr = (assignStatement->lhs().size() > 0 && isFixedPointVariable((assignStatement->lhsIdentifiers())[0])) ? "1" : "0";

			string printAssignStr;

			vector<SliceName *> outputNames = assignStatement->lhsSliceNames();
			...
			case ktBinaryExpression:
			{
				...
				{
					printAssignStr = "printAssignStatement(fileID1, ";
					...
					//								if (!leftOpName->isDummy())
					//								{
					//									printAssignStr += dumpMatlabExpression(leftOpName->prefix())               + "', ";
					//									printAssignStr += dumpMatlabExpression(leftOpName->suffix()[0]) + ", '";
					//									printAssignStr += dumpMatlabExpression(leftOpName->suffix()[0]) + "', ";
					//									printAssignStr += dumpMatlabExpression(leftOpName, true)				   + ", ";
					//								}
					//								else if (leftOpName->kind() == ktSliceName)
					//								{
					//									NumericLiteral * objectValue = leftOpName->prefix()->object()->value();
					//
					//									printAssignStr += dumpMatlabExpression(objectValue) + "', ";
					//									printAssignStr += dumpMatlabExpression(objectValue) + ", '";
					//									printAssignStr += dumpMatlabExpression(objectValue) + "', ";
					//									printAssignStr += dumpMatlabExpression(objectValue) + ", ";
					//								}
					...
					//								if (!rightOpName->isDummy())
					//								{
					//									printAssignStr += dumpMatlabExpression(rightOpName->prefix())               + "', ";
					//									printAssignStr += dumpMatlabExpression(rightOpName->suffix()[0]) + ", '";
					//									printAssignStr += dumpMatlabExpression(rightOpName->suffix()[0]) + "', ";
					//									printAssignStr += dumpMatlabExpression(rightOpName, true)                   + ", ";
					//								}
					//								else if (rightOpName->kind() == ktSliceName)
					//								{
					//									NumericLiteral * objectValue = rightOpName->prefix()->object()->value();
					//
					//									printAssignStr += dumpMatlabExpression(objectValue) + "', ";
					//									printAssignStr += dumpMatlabExpression(objectValue) + ", '";
					//									printAssignStr += dumpMatlabExpression(objectValue) + "', ";
					//									printAssignStr += dumpMatlabExpression(objectValue) + ", ";
					//								}
					//								else // case of Add with constant
					//								{
					//									printAssignStr += dumpMatlabExpression(rightOpName) + "', ";
					//									printAssignStr += dumpMatlabExpression(rightOpName) + ", '";
					//									printAssignStr += dumpMatlabExpression(rightOpName) + "', ";
					//									printAssignStr += dumpMatlabExpression(rightOpName) + ", ";
					//								}

					if (rightOpName->kind() == ktSliceName)
					{
						NumericLiteral *objectValue = rightOpName->prefix()->object()->value();
					...
					printAssignStr += "'";
					printAssignStr += dumpMatlabExpression(outputName->prefix()) + "', ";
					printAssignStr += dumpMatlabExpression(outputName->suffix()[0]) + ", '";
					printAssignStr += dumpMatlabExpression(outputName->suffix()[0]) + "', ";
					printAssignStr += dumpMatlabExpression(outputName, true) + ", '";
					printAssignStr += binaryExprssion->operatorStr() + "', ";
					printAssignStr += isFixedPointStr + ");";

					// Overwriting matlabExprStr in case of dummy names
					if (leftOpName->isDummy() || rightOpName->isDummy())
					{
						matlabExprStr = dumpMatlabExpression(assignStatement->lhsSliceNames()[0]) + " = ";
				...
				{
					printAssignStr = dumpMatlabPrimitiveFunc((PrimitiveFunctionCall *)((BinaryExpression *)assignStatement->rhs())->rightOperand(), outputNames, isFixedPointStr);
				}
			}
			break;

			case ktPrimitiveFunctionCall:
				printAssignStr = dumpMatlabPrimitiveFunc((PrimitiveFunctionCall *)assignStatement->rhs(), outputNames, isFixedPointStr);
				break;
			...
			string printAssignWTStr = "";
			string printResultStr = "";
			if (outputNames.size() > 0)
			{
				for (auto outputName : outputNames)
				{
					printAssignWTStr += "printAssignStatementWT(fileID1, ";
					printResultStr += "printResult(fileID3, ";
					printAssignWTStr += "'";
					printAssignWTStr += dumpMatlabExpression(outputName->prefix()) + "', ";
					printAssignWTStr += dumpMatlabExpression(outputName->suffix()[0]) + ", '";
					printAssignWTStr += dumpMatlabExpression(outputName->suffix()[0]) + "', ";
					printAssignWTStr += dumpMatlabExpression(outputName, true) + ", ";
					printAssignWTStr += isFixedPointStr + ");\n";
			...
			// COMMENTED TO DISABLE USELESS LOGGING
			//WriteLine(printAssignStr);

			WriteLine(matlabExprStr);

			// COMMENTED TO DISABLE USELESS LOGGING
			//WriteLine(printAssignWTStr);
		...
		{
			SliceName *lhsSliceName = dynamic_cast<SliceName *>((assignStatement->lhsSliceNames())[0]);
			SliceName *rhsSliceName = dynamic_cast<SliceName *>((assignStatement->rhsSliceNames())[0]);

			if (!lhsSliceName || !rhsSliceName)
				LOG(FATAL) << ("dumpMatlabStatement: Improper storage transfer statement!");

			string printAssignStr = "printStorageTransfer(fileID1, '";

			printAssignStr += dumpMatlabExpression(rhsSliceName->prefix()) + "', ";
			printAssignStr += dumpMatlabExpression(rhsSliceName->suffix()[0]) + ", '";
			printAssignStr += dumpMatlabExpression(rhsSliceName->suffix()[0]) + "', ";
			printAssignStr += dumpMatlabExpression(rhsSliceName) + ", '";

			printAssignStr += dumpMatlabExpression(lhsSliceName->prefix()) + "', ";
			printAssignStr += dumpMatlabExpression(lhsSliceName->suffix()[0]) + ", '";
			printAssignStr += dumpMatlabExpression(lhsSliceName->suffix()[0]) + "', ";
			printAssignStr += dumpMatlabExpression(lhsSliceName) + ");";
			...
			{
				string isFixedPointStr = isFixedPointVariable((assignStatement->lhsIdentifiers())[0]) ? "1" : "0";
...
// This method gets a DRRA cell and finds the scheduled cycles that
// a memory read access (write access to register file) is active.
// The return value indicates that if any skip cycles found or not.
bool VDump::dumpProfilerSkipWtCyclesForACell(BIR::DRRACell *drraCell_)
{
	vector<Instruction *> &instr_list = drraCell_->instr_list;

	std::stack<LoopHeaderInstruction *> activeLoops;
	LoopHeaderInstruction *currLoop = 0;
	LoopList loopList;
	bool isFirstItem = true;
	int currIterationNo = 0;
	int currLoopExecCycle = 0;
	int counter = 0;

	activeLoops.push(0);
	string lineStr;

	int accumulatedExec = 0;

	vector<LoopHeaderInstruction *> curr_loop_header_instrs;
	for (int index = 0; index < instr_list.size(); ++index)
	{
		Instruction *currInst = instr_list[index];

		switch (currInst->kind())
		{
		case bktLoopHeaderInstruction:
		{
			if (curr_loop_header_instrs.size() > 0)
			{
				curr_loop_header_instrs.back()->loopExecution = currInst->minScheduledClkCycle - curr_loop_header_instrs.back()->maxScheduledClkCycle;
			}
			curr_loop_header_instrs.push_back(static_cast<LoopHeaderInstruction *>(currInst));
		}
		break;
		...
		case bktLoopTailInstruction:
		{
			if (curr_loop_header_instrs.size() > 0)
			{
				curr_loop_header_instrs.back()->loopExecution = currInst->minScheduledClkCycle - curr_loop_header_instrs.back()->maxScheduledClkCycle + 1;
			}
			curr_loop_header_instrs.pop_back();
		}
		break;
		}
	}
	...
	for (int index = 0; index < instr_list.size(); ++index)
	{
		...
		case bktLoopHeaderInstruction:
		{
			currLoop = (LoopHeaderInstruction *)currInst;
			activeLoops.push(currLoop);
			createLoopList(currLoop, loopList);
		}
		break;

		case bktLoopTailInstruction:
		{
			accumulatedExec -= activeLoops.top()->loopExecution;
			activeLoops.pop();
			currLoop = activeLoops.top();
			createLoopList(currLoop, loopList);
		}
		break;
		...
		case bktSRAMInstruction:
		{
			SRAMInstruction *sramInst = (SRAMInstruction *)currInst;

			if (sramInst->isRead)
				loopList.size() > 0 ? dumpProfilerSkipCyclesForAnInstInLoop(sramInst, loopList, 0, counter, accumulatedExec, lineStr) : dumpProfilerSkipCyclesForAnInst(sramInst, counter, accumulatedExec, lineStr);
		}
		break;
		}
	}
...
// This method get a SRAM read instruction and dumps the skip cycles to the profiler associative array.
void VDump::dumpProfilerSkipCyclesForAnInst(BIR::SRAMInstruction *sramInst_, int &counter_, int accumulatedExec_, string &lineStr_)
{
	for (int cycles = sramInst_->minScheduledClkCycle; cycles <= sramInst_->maxScheduledClkCycle; ++cycles)
	{
		if (counter_ != 0)
			lineStr_ += ", ";
		else
		...
		lineStr_ += to_string(cycles + accumulatedExec_) + ":0";
...
// This method gets a SRAM read instruction and according to its position in a nested loop,
// it finds the active cycles, in order to skip them in the profiling.
// This method is called recursively for nested loops.
// Note that in LoopList, the loops are in descending order of their degrees. So the last element is the loop containing the SRAM instruction.
void VDump::dumpProfilerSkipCyclesForAnInstInLoop(BIR::SRAMInstruction *sramInst_, LoopList &loopList_, int currListIndex_, int &counter_, int &accumulatedExec_, string &lineStr_)
{
	LoopHeaderInstruction *currLoop = loopList_.at(currListIndex_);

	bool isSramLoop = currListIndex_ == (int)loopList_.size() - 1;

	for (int index = 0; index < currLoop->iterationNumber; ++index)
	{
		if (!isSramLoop)
		{
			dumpProfilerSkipCyclesForAnInstInLoop(sramInst_, loopList_, currListIndex_ + 1, counter_, accumulatedExec_, lineStr_);
		}

		if (currListIndex_ == 0 || index != currLoop->iterationNumber - 1)
		{
			dumpProfilerSkipCyclesForAnInst(sramInst_, counter_, accumulatedExec_, lineStr_);
			accumulatedExec_ += (isSramLoop ? getInnerLoopExecution(currLoop, 1, true) : currLoop->loopExecution);
		}
	}
}
...
{
	//! Commented in order to use autotest to handle the location of fabric description.
	//	WriteLine("if [file exists work] {");
	//	WriteLine("	vdel -all");
	//	WriteLine("}");
	//	WriteLine("vlib work");
	//	WriteLine("");
	//
	//	WriteLine("vcom ../../../../fabric/mtrf/util_package.vhd");
	//	WriteLine("vcom ../../../../fabric/hw_setting.vhd");
	//	WriteLine("vcom -mixedsvvh ../../../../fabric/mtrf/top_consts_types_package.vhd");
	//	WriteLine("vcom -F ../../../../fabric/fabric_files.f");

	WriteLine("vcom -mixedsvvh const_package_" + InputFileName() + ".vhd");

	//! Commented in order to use autotest to handle the location of fabric description.
	//	WriteLine("vlog -F ../../test_util/test_util_files.f");

	WriteLine("vlog profiler_" + InputFileName() + ".sv");
	WriteLine("vcom testbench_" + InputFileName() + ".vhd");
	WriteLine("");
	//WriteLine("vopt testbench_" + InputFileName() + " -o testbench_" + InputFileName() + "_opt");
	//WriteLine("vsim testbench_" + InputFileName() + "_opt");
...
{
	//! Commented in order to use autotest to handle the location of fabric description.
	//	WriteLine(".main clear");
	//	WriteLine("");
	//	WriteLine("# Create library");
	//	WriteLine("");
	//	WriteLine("if [file exists work] {");
	//	WriteLine("	vdel -all");
	//	WriteLine("}");
...
/* COMMENTED BY Yu Yang
 * Use instr_list to store synchronized instructions.
 *
void VDump::dumpSynchronizedInstructions(BIR::DRRACluster *drraCluster_)
{
	dumpTitle("DRRA Synchronized Instructions", "#");
	...
	for (int rowIndex = 0; rowIndex < drraCluster_->rowNum(); ++rowIndex)
	{
		for (int colIndex = 0; colIndex < drraCluster_->colNum(); ++colIndex)
		{
			InstructionList *instList = &drraCluster_->getDRRACell(rowIndex, colIndex)->optimizedInstructions();
...
void VDump::dumpTimingModel(BIR::DRRACluster *drraCluster_, VIR::MainProgram *mainProgram_)
{
	...
	for (int stIndex = 0; stIndex < stList.size(); ++stIndex)
	{
		...
		if (assignStatement && assignStatement->type() == atDeclarative)
		{
			...
			{
				string variableName = assignStatement->lhsIdentifiers()[0]->name();
				string variableType;
...
{
	return identifier_->object()->isFixedPointVariable();
}
