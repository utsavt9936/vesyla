...
public:
  ...
  void update_var(string name_, int bind_vertex_id_, int bind_vertex_port_, string domain_signature_, Edge::EdgeType edge_type_ = Edge::SCALAR_DATA_SIG, VIR::Coordinate coord_ = VIR::Coordinate(), bool en_compression_ = false, bool isfixed_ = false, int start_vertex_id_ = -1, int size_vertex_id_ = -1);
  void delete_vars(string domain_signature_);
  int get_vertex_id(string name_);
  int get_vertex_port(string name_);
  bool get_en_compression(string name_);
