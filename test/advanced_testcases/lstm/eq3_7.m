...
X_reg_06 = zeros(1, REG_DEPTH/2);  %! RFILE [0,6]
...
X_reg_16 = zeros(1, REG_DEPTH/2);  %! RFILE [1,6]
...
W_reg_06 = zeros(1, REG_DEPTH/2);  %! RFILE [0,6]
...
W_reg_16 = zeros(1, REG_DEPTH/2);  %! RFILE [1,6]
B_reg_0  = zeros(1, REG_DEPTH);    %! RFILE [0,7]
B_reg_1  = zeros(1, REG_DEPTH);    %! RFILE [1,7]
...
for i = 1 : HIDDEN_SIZE/REG_DEPTH/2
	...
	for j = 1 : REG_DEPTH
		for k = 1 : CONCAT_SIZE_2/COMPUTING_DRRA_COL/COMPUTING_DRRA_ROW/REG_DEPTH
			...
			X_reg_06 = X_sram(REG_DEPTH/2*6+1:REG_DEPTH/2*7);
			...
			X_reg_16 = X_sram(REG_DEPTH/2*6+1:REG_DEPTH/2*7);
			...
			W_reg_06 = W0_sram(REG_DEPTH/2*6+1:REG_DEPTH/2*7);
			...
			W_reg_16 = W1_sram(REG_DEPTH/2*6+1:REG_DEPTH/2*7);
			...
			W_reg_06(1) = sum(X_reg_06 .* W_reg_06); %! DPU [0,6]
			...
			W_reg_16(1) = sum(X_reg_16 .* W_reg_16); %! DPU [1,6]
			...
			W_reg_06(1) = W_reg_06(1) + W_reg_05(1); %! DPU [0,6]
			W_reg_16(1) = W_reg_16(1) + W_reg_15(1); %! DPU [1,6]
			B_reg_0(j) = B_reg_0(j) + W_reg_06(1);   %! DPU [0,7]
			B_reg_1(j) = B_reg_1(j) + W_reg_16(1);   %! DPU [1,7]
		end
	end
	B_reg_0 = __sl_sigmoid(B_reg_0);               %! DPU [0,7]
	B_reg_1 = __sl_sigmoid(B_reg_1);               %! DPU [1,7]
