...
% Features:
%  - Using an address function dependent to four levels of nested loop
%  - Need a sequencer of size 256
	...
	for col = 1 : WIDTH-1
		for i = 1 : K_HEIGHT
			for j = 1 : K_WIDTH
				...
				addr = (row+i-2)*WIDTH + col + j - 1;
				
				temp(1) = inp(addr) .* kernel((i-1)*K_WIDTH + j); %! DPU[0,0]
